    ...
    request.get(mimemiUrl, (err, response, body) => {
      ...
      switch (mimemiType) {
        ...
        case 'clash':
          ...
          body.proxies = body.proxies.filter(
            (proxy) => !excludeReg.test(proxy.name),
          );
          body[PG].map((item) => {
            item.proxies = item.proxies.filter(
              (name) => !excludeReg.test(name),
            );
          });
        ...
        case 'surge':
          ...
          body = [
            firstLine,
            ini.stringify(body, { whitespace: true }),
            '[Rule]',
            ruleLines,
          ].join('\n');
          break;
...
function ssrParser(url, reg) {
  ...
  if (urlObj.protocol === 'ssr:') {
    urlObj = URL.parse(
      '//' + base64_decode(url.replace(urlObj.protocol + '//', '')),
    );
